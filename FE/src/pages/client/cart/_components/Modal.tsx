...
const ModalCart = ({ open, onClose, idCart, onUpdateAttributes }: Props) => {
    const { token } = useAuth();
    ...
    // console.log("Selected Attributes:", activeAttributes);// Xem các thuộc tính đã chọn


    const { data: cartAttribute, isFetching } = useQuery({
        queryKey: ['cartAttribute', idCart],
        queryFn: async () => {
            let res = await instance.get(`/cart/${idCart}`, {
                headers: {
    ...
    return (
        <AntModal open={open} onCancel={onClose} footer={false} closable={false} maskClosable={false}>
                        ...
                        <div key={e.attribute}>
                            ...
                            <div className={`flex mt-3 gap-2 ${e.attribute === 'Color' ? 'flex-wrap' : 'grid grid-cols-5 sm:grid-cols-7'}`}>
                                {e.attributeValue.map((item) => (
                                    <div
                                        key={item.id}
                                        className={`relative flex-1 max-w-[75px] h-8 sm:h-8 rounded-full border-2 cursor-pointer border-primary-6000 dark:border-primary-500 ${activeAttributes[e.attribute] === item.id ? 'border-black' : ''}`}
            ...
            <button onClick={handleConfirm} className="nc-Button relative right-2 h-14 w-64 inline-flex items-center justify-center rounded-full text-sm sm:text-base font-medium sm:py-3.5 sm:px-2 lg:px-2 shadow-xl focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-6000 dark:focus:ring-offset-0 animate-bounce focus:animate-none hover:animate-none text-md mt-8 ml-28 border bg-[#56cfe1] text-white">
