...
class StoreVoucherRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'discount_type' => 'required|in:fixed,percent',
            'discount_value' => [
                'required_if:discount_type,fixed,percent', // Bắt buộc nếu là fixed hoặc percent
                'numeric', // Phải là số
                'min:0', // Không được nhỏ hơn 0
                function ($attribute, $value, $fail) {
                    if ($this->discount_type == 'percent' && $value > 100) {
            ...
            'min_order_value' => [
                ...
                function ($attribute, $value, $fail) {
                    if ($this->discount_type == 'fixed' && $value < $this->discount_value) {
                        $fail('The ' . $attribute . ' must be greater than or equal to discount value when discount type is fixed.');
                    }
                }
            ],
